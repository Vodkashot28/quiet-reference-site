...
set -e ZONE="$1" HASH="$2" ([ ! -z "$DNSIMPLE_TOKEN" ] && [ ! -z "$ZONE" ] && [ ! -z "$HASH" ]) \ || (echo "Usage: DNSIMPLE_TOKEN=dnsimple_a_w3zCFhOH5exXWFOBW6uKS94cCH0KuIed ./dnslink.sh quietreference.xyz bafybeicbaydrktruy4besipc7fkculoiyrtg5vuq76zy2czl2fksxblpom " && exit 1) RECORD_NAME="_dnslink" RECORD_TTL=120
